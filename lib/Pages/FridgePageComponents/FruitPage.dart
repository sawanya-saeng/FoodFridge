...
class fruit_page extends StatefulWidget {
  @override
  _fruit_page createState() => _fruit_page();
}
int click;

class _fruit_page extends State<fruit_page> with TickerProviderStateMixin{
  ...
  Future getMeat() async {
    ...
    _db.collection('Fridge').where('uid', isEqualTo: user.uid).where('type', isEqualTo:'fruit').orderBy('date', descending: false).snapshots().listen((docs) {
      tmp = docs.documents;
      setState(() {
        ingres = tmp;
        items.clear();
